...
**Results**: The first two PLS components (C1 and C2) extracted 
from the two models explained `r Y_explvar_issi2`-`r Y_explvar_isi`% of
...
Only a subset of TGFA strongly correlated with the component scores (i.e. 
`r tg_contrib`). A lower PLS score (indicating higher mol% of these specific TGFA)
associated with `r estrange$estimate[1]`% lower ISI and `r estrange$estimate[2]`% 
lower ISSI-2.

**Conclusions**: A cluster of TGFA had strong negative associations with the
outcomes. These TGFA (e.g. 16:1n-7) are reported as markers of de novo
lipogenesis from carbohydrates and can be lipotoxic. Our results suggest only a
subset of FA in serum may underlie the association between lipids and glucose
dysregulation, and that these lipids are possible mediators between the
associations of carbohydrate intake and diabetes risk.
